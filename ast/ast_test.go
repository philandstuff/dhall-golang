...
var _ = DescribeTable("Shift",
	...
	Entry("Shift(1, x, {}) = {}", Shift(1, x(0), Record{}), Record{}),
	Entry("Shift(1, x, {=}) = {=}", Shift(1, x(0), RecordLit{}), RecordLit{}),
	Entry("Shift(1, x, {foo: x}) = {foo: x@1}", Shift(1, x(0), Record{"foo": x(0)}), Record{"foo": x(1)}),
	Entry("Shift(1, x, {foo = x}) = {foo = x@1}", Shift(1, x(0), RecordLit{"foo": x(0)}), RecordLit{"foo": x(1)}),
...
var _ = DescribeTable("Subst",
	...
	Entry("Subst(x, 3, {}) = {}", Subst(x(0), NaturalLit(3), Record{}), Record{}),
	Entry("Subst(x, 3, {=}) = {=}", Subst(x(0), NaturalLit(3), RecordLit{}), RecordLit{}),
	Entry("Subst(x, Natural, {foo: x}) = {foo: Natural}", Subst(x(0), Natural, Record{"foo": x(0)}), Record{"foo": Natural}),
	Entry("Subst(x, 3, {foo = x}) = {foo = 3}", Subst(x(0), NaturalLit(3), RecordLit{"foo": x(0)}), RecordLit{"foo": NaturalLit(3)}),
	Entry("Subst(x, {foo = 4}, x.foo) = {foo = 4}.foo", Subst(x(0), RecordLit{"foo": NaturalLit(4)}, Field{x(0), "foo"}), Field{RecordLit{"foo": NaturalLit(4)}, "foo"}),
...
var _ = Describe("Normalize", func() {
	DescribeTable("Normalize",
		...
		Entry("{} » {}", Record{}, Record{}),
		Entry("{=} » {=}", RecordLit{}, RecordLit{}),
		Entry("{foo = 3 + 5} » {foo = 8}", RecordLit{"foo": NaturalPlus(NaturalLit(3), NaturalLit(5))}, RecordLit{"foo": NaturalLit(8)}),
		Entry("{foo = 3 + 5}.foo » 8", Field{RecordLit{"foo": NaturalPlus(NaturalLit(3), NaturalLit(5))}, "foo"}, NaturalLit(8)),
...
var _ = Describe("AlphaNormalize", func() {
	DescribeTable("AlphaNormalize",
		...
		Entry(`{i = λ(x : Natural) → x} » {i = λ(x : Natural) → x}`,
			RecordLit{"i": &LambdaExpr{"x", Natural, x(0)}},
			RecordLit{"i": &LambdaExpr{"_", Natural, __(0)}},
		),
		Entry(`(let y = 5 in {x = 5}).x » (let _ = 5 in {x = 5}).x`,
			Field{MakeLet(RecordLit{"x": NaturalLit(5)}, Binding{Variable: "y", Value: NaturalLit(5)}), "x"},
			Field{MakeLet(RecordLit{"x": NaturalLit(5)}, Binding{Variable: "_", Value: NaturalLit(5)}), "x"},
		),
