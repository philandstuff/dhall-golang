...
var _ = DescribeTable("Shift",
	...
	Entry("Shift(1, x, {foo: x}) = {foo: x@1}", Shift(1, x(0), Record(map[string]Expr{"foo": x(0)})), Record(map[string]Expr{"foo": x(1)})),
	Entry("Shift(1, x, {foo = x}) = {foo = x@1}", Shift(1, x(0), RecordLit(map[string]Expr{"foo": x(0)})), RecordLit(map[string]Expr{"foo": x(1)})),
...
var _ = DescribeTable("Subst",
	...
	Entry("Subst(x, Natural, {foo: x}) = {foo: Natural}", Subst(x(0), Natural, Record(map[string]Expr{"foo": x(0)})), Record(map[string]Expr{"foo": Natural})),
	Entry("Subst(x, 3, {foo = x}) = {foo = 3}", Subst(x(0), NaturalLit(3), RecordLit(map[string]Expr{"foo": x(0)})), RecordLit(map[string]Expr{"foo": NaturalLit(3)})),
...
var _ = Describe("IsFreeIn", func() {
	DescribeTable("`x` is free examples",
		...
		Entry("x", MkVar("x")),
		Entry("x+3", NaturalPlus(MkVar("x"), NaturalLit(3))),
...
var _ = Describe("Normalize", func() {
	DescribeTable("Normalize",
		...
		Entry("List Integer", Apply(List, Integer), Apply(List, Integer)),
		Entry("Optional Integer", Apply(Optional, Integer), Apply(Optional, Integer)),
		Entry("None Integer", Apply(None, Integer), Apply(None, Integer)),
		...
		Entry("[] : List ((λ(x : Type) → x) Natural) → [] : List Natural",
			EmptyList{Apply(&LambdaExpr{"x", Type, x(0)}, Natural)},
			EmptyList{Natural}),
...
var _ = Describe("AlphaNormalize", func() {
	DescribeTable("AlphaNormalize",
		...
		Entry("(λ(x : Natural) → x) 3 » (λ(_ : Natural) → _) 3",
			Apply(&LambdaExpr{"x", Natural, x(0)}, NaturalLit(3)),
			Apply(&LambdaExpr{"_", Natural, __(0)}, NaturalLit(3))),
		...
		Entry("[] : List ((λ(x : Type) → x) Natural) → [] : List ((λ(_ : Type) → x) Natural)",
			EmptyList{Apply(&LambdaExpr{"x", Type, x(0)}, Natural)},
			EmptyList{Apply(&LambdaExpr{"_", Type, Var{"_", 0}}, Natural)}),
