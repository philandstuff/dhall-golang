...
var _ = Describe("TypeCheck in empty context", func() {
	...
	DescribeTable("Type errors",
		...
		Entry("record types can't mix types and kinds", Record{"foo": Natural, "bar": Type}),
		Entry("record types can't mix kinds and sorts", Record{"foo": Type, "bar": Kind}),
		Entry("record literals can't mix types and kinds", RecordLit{"foo": NaturalLit(3), "bar": Natural}),
		Entry("record literals can't mix kinds and sorts", RecordLit{"foo": Natural, "bar": Type}),
