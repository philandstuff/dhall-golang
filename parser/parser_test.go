...
''`,
	...
	DescribeTable("imports", ParseAndCompare,
		...
		PEntry("remote with query string", `https://example.com/foo?bar=baz&fred=jim`, Embed(MakeRemoteImport("https://example.com/foo?bar=baz&fred=jim", Code))),
		PEntry("remote with userinfo", `https://foo:bar@example.com/foo`, Embed(MakeRemoteImport("https://foo:bar@example.com/foo", Code))),
		PEntry("remote with port", `https://example.com:8080/foo`, Embed(MakeRemoteImport("https://example.com:8080/foo", Code))),
		PEntry("remote with IPv4 address", `https://127.0.0.1/foo`, Embed(MakeRemoteImport("https://127.0.0.1/foo", Code))),
		PEntry("remote with IPv6 address", `https://[cafe:d00d::1234]/foo`, Embed(MakeRemoteImport("https://[cafe:d00d::1234]/foo", Code))),
